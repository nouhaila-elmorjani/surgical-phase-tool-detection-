...
from config import IMAGE_ROOT, IMAGE_SIZE, WINDOW_SIZE, NUM_PHASE_CLASSES, NUM_TOOL_CLASSES
...
def build_video_index(samples: List[Dict]) -> Dict[str, List[int]]:
    ...
    # sort indices within each video by frame_number
...
class MultiTaskWindowDataset(Dataset):
    ...
    def _get_window_indices(self, center_idx: int) -> List[int]:
        ...
        # find position of center_idx in idxs (linear search is fine: only 2-3k frames per video)
