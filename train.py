...
from config_loader import load_config, set_global_seed
from dataset import MultiTaskWindowDataset, PHASE_TO_ID, TOOL_COLUMNS
from models.resnet_multitask import PhaseToolNet
from hierarchy.phase_tool_mask import build_phase_tool_mask, apply_phase_mask_to_logits
from metrics import phase_metrics, tool_metrics
...
def train_one_epoch(model, loader, optimizer, phase_criterion, tool_criterion, phase_tool_mask):
    ...
    for frames, phase_target, tool_target in loader:
        ...
        # Hierarchical tool loss: mask with ground-truth phase
...
def evaluate(model, loader, phase_criterion, tool_criterion, phase_tool_mask) -> Tuple[dict, dict]:
    ...
    with torch.no_grad():
        for frames, phase_target, tool_target in loader:
            ...
            all_tool_logits.append(tool_logits.cpu())  # keep raw logits for hierarchy comparison
    ...
    # Tool metrics WITHOUT masking
    tool_stats_no_mask = tool_metrics(all_tool_logits, all_tool_targets, threshold=0.5)

    # Tool metrics WITH predicted-phase masking (hierarchical inference)
